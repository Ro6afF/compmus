...
pub fn construct_probalility_table(a: &MidiFile) -> HashMap<u8, Vec<(u8, f32)>> {
    let mut res1 = HashMap::<u8, Vec<u8>>::new();
    for i in &a.chunks {
        match i {
            Chunk::Track(x) => {
                                ...
                                for k in (j + 1)..x.events.len() {
                                    match &x.events[k] {
                                        Event::Midi(z) => if z.message_type >> 4 == 0b1001 {
                                            (*res1.entry(y.data1).or_insert(Vec::<u8>::new()))
                                                .push(z.data1);
                                            break;
                                        },
                                        _ => {}
    ...
    let mut res = HashMap::<u8, Vec<(u8, f32)>>::new();
    for (k, v) in res1 {
